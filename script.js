document.addEventListener("DOMContentLoaded", () => {
  ...
  function fadeInMusic(audio){
    let vol=0;
    const fade=setInterval(()=>{ vol+=0.02; audio.volume=Math.min(vol,0.6); if(vol>=0.6) clearInterval(fade); },120);
  }
  ...
  if(envelope && letterText){
    ...
    function typeWriter(){
      ...
      if(fullText[index]==="<"){ const tagEnd=fullText.indexOf(">",index); const tag=fullText.substring(index,tagEnd+1);
        letterText.innerHTML+=tag; index=tagEnd+1; setTimeout(typeWriter,0); return;
      }
...
if(heartsContainer){setInterval(()=>{
  ...
  heartsContainer.appendChild(heart); setTimeout(()=>heart.remove(),14000);
},900);}
