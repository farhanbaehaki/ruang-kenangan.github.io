    ...
    function typeWriter(){
      ...
      if(fullText[index]==="<"){
        ...
        if(tag==="<br>" && fullText.substring(index,index+4)==="<br>"){
          letterText.innerHTML += "<br>"; index += 4; setTimeout(typeWriter,800); return;
        }
        setTimeout(typeWriter,0); return;
      }

      const char = fullText.charAt(index);
      letterText.innerHTML += char; index++;
...
if(heartsContainer){ setInterval(()=>{
  ...
  heart.style.left = Math.random()*100 + "vw"; heart.style.animationDuration = 8+Math.random()*6 + "s";
